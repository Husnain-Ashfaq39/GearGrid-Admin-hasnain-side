...
const ContactList = () => {
  ...
  const fetchAllContacts = useCallback(async () => {
    ...
    try {
      const allContacts = await axios.get(`http://localhost:5001/contactus/all`);
      console.log('contacts '+JSON.stringify(allContacts)); // Continue fetching if the limit was reached
  ...
  const handleMarkAsRead = async (contact) => {
    try {
      const updatedContact = await axios.put(`http://localhost:5001/contactus/update-read/${contact._id}`,
        { read: true },
        {
          headers: {
      ...
      console.log('contact ' + JSON.stringify(updatedContact));
      setContacts(
        contacts.map((c) =>
